...
function slice(foods, one, two) {
    modifiedFood = foods.slice(one, two);
    return modifiedFood;
}
...
let foods1 = ['pizza', 'burger', 'fingerChips', 'donuts', 'springRoll'];
let modifiedFood = foods1.splice(2, 0, 'noodles', 'icecream');
modifiedFood = foods1;
...
function isEven(numberArray) {
    let num = numberArray.filter(function (num1) {
        if (num1 % 2 == 0) {
            return num1;
        }
    });
...
modifiedArray = isEven(numberArray);
...
modifiedArray = isPrime(numberArray);
...
modifiedArray = nonPrime(numberArray);
...
let numberArray = [12, 324, 213, 4, 2, 3, 45, 4234];
num1 = numberArray.filter(number => number % 2 == 0);
...
let num1 = myArray.map(number => (number * number));
console.log(num1);
...
let multiplication = numbers.reduce(function (innitial, item) {
    return innitial * item;
}, 1);
multiplication;
